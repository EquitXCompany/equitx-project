...
async function fetchPriceHistory(assetSymbol: string, startDate: Date, endDate: Date): Promise<PriceHistory[]> {
...
async function fetchLatestPrice(assetSymbol: string): Promise<PriceHistory> {
  const { data } = await apiClient.get(`/api/price-history/latest/${assetSymbol}`);
  return {
